...
fn get_articles_links(tab: &Arc<Tab>) -> Result<Vec<String>> {
    Ok(tab
        .find_elements(".rectangle-story-item, .rectangle-story-group__article-hero")
        .context("find_elements links")?
        .iter()
        .map(|el| {
            let mut link = el
                .get_attribute_value("data-bbc-result")
                .unwrap()
                .expect("a data-bbc-result");
            if !link.starts_with("http") {
                link.insert_str(0, "https://www.bbc.com");
            }
            link
        })
        .collect())
...
pub fn get_news(opts: GetNewsOpts) -> Result<()> {
    ...
    tab.navigate_to("https://www.bbc.com/travel")
        .context("navigate_to")?
        .wait_until_navigated()
        .context("wait_until_navigated")?;
    tab.evaluate(
        "document.querySelector('.around-the-bbc-swimlane').remove()",
        false,
    )?;
